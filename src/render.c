...
void render_con(Con *con, bool render_fullscreen, bool already_inset) {
    ...
    if (!already_inset && should_inset) {
        ...
        Rect inset = (Rect){
            has_adjacent_container(con, D_LEFT) ? gaps.inner : (gaps.outer + gaps.horizontal),
            has_adjacent_container(con, D_UP) ? gaps.inner : (gaps.outer + gaps.vertical),
            has_adjacent_container(con, D_RIGHT) ? -gaps.inner : -(gaps.outer + gaps.horizontal),
            has_adjacent_container(con, D_DOWN) ? -gaps.inner : -(gaps.outer + gaps.vertical)};
        inset.width -= inset.x;
