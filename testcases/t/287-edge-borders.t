...
# window visible with outer gaps
#####################################################################
...
ok(@tiled == 1, 'one tiled container opened');
is($tiled[0]->{current_border_width}, 2, 'tiled current border width set to 2');
is($tilewindow->rect->width, $tiled[0]->{rect}->{width} - 2*2, 'single tiled border width 0');

exit_gracefully($pid);

#####################################################################
# 6: check that the borders are hidden on a workspace with one tiled
# window visible and no outer gaps
#####################################################################
