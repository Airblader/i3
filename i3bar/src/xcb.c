...
void refresh_statusline(bool use_short_text) {
    ...
    TAILQ_FOREACH(block, &statusline_head, blocks) {
        ...
        if (block->border || block->background || block->urgent) {
            ...
            /* Draw the border. */
            cairo_set_source_color(&statusline_surface, border_color);
            cairo_rectangle(statusline_surface.cr, x, logical_px(1),
            ...
            cairo_set_source_color(&statusline_surface, bg_color);
            cairo_rectangle(statusline_surface.cr,
        ...
        draw_text(block->full_text, statusline_surface.id, statusline_surface.gc,
